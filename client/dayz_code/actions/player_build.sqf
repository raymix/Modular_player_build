...
if (!DZE_customBuildSystem) then {
	...
	if (_hasrequireditem and DZE_ActionInProgress) then {
		...
		if (DZE_snapBuilding) then {
			_controls = [_object, _isAllowedUnderGround, _location1, _position, _objectHelper] call snap_build_controls;
		} else {
			//give player controls (rotate, adjust height, distance checks, key presses)
			_controls = [_object, _isAllowedUnderGround, _location1, _position] call player_build_controls;
		};
		//define items collected from function
